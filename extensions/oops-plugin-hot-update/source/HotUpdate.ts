import { PACKAGE_NAME } from "./hooks";
...
// ---- 扩展注入热更新脚本结束 ----` + mainScript;
    ...
    private init(options) {
        ...
        console.log('[' + PACKAGE_NAME + '] 生成 project.manifest 成功');
    }

    private manifest(options) {
        var packageOptions = options.packages[PACKAGE_NAME];
        ...
        fs.writeFileSync(destManifestPath, JSON.stringify(manifest));
        console.log('[' + PACKAGE_NAME + '] 已成功生成清单文件');
        ...
        console.log('[' + PACKAGE_NAME + '] 已成功生成版本文件');
